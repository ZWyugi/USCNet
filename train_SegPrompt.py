...
class TrainerSeg:
    ...
    def __call__(self):
        if self.args.phase == 'train':
            for epoch in tqdm(range(self.epochs)):
                ...
                if epoch % 1 == 0:
                    self.evaluate()
            self.print_metrics(self.best_metrics, prefix='The Best metrics in epoch {}'.format(self.best_dice_epoch))
        else:
            self.evaluate()
    ...
    def calculate_metrics(self, seg, mask):
        with torch.no_grad():
    ...
    def calculate_loss(self, seg, mask):
        # mask[mask == 0] = -1
    ...
    def get_meters(self):
        meters = {
            'loss': AverageMeter(), 'dice': AverageMeter()
        }
        return meters
    ...
    def train_one_epoch(self):
        ...
        for inx, (img, mask, label, clinical) in tqdm(enumerate(self.train_loader), total=len(self.train_loader)):
            ...
            seg = self.model(img)
            dice_loss = self.calculate_loss(seg, mask)
            loss = dice_loss
            self.optimizer.zero_grad()
            loss.backward()
            self.optimizer.step()

            dice = self.calculate_metrics(seg.cpu(), mask.cpu())
            self.update_meters(
    ...
    def evaluate(self):
        ...
        with torch.no_grad():  # 禁用梯度计算
            for inx, (img, mask, label, clinical) in tqdm(enumerate(self.test_loader), total=len(self.test_loader)):
                ...
                dice_loss = self.calculate_loss(seg, mask)
                loss = dice_loss
                dice = self.calculate_metrics(seg, mask)
                self.update_meters(
                    [meters[i] for i in meters.keys()],
                    [loss, dice])
                if self.args.phase != 'train':
        ...
        if self.args.phase == 'train':
            # 检查并保存最佳模型
            if meters['dice'].avg > self.best_dice:
                self.best_dice_epoch = self.epoch
                self.best_dice = meters['dice'].avg
                self.best_metrics = meters
                with open(os.path.join(os.path.dirname(self.args.save_dir), 'best_dice_metrics.json'), 'w')as f:
                ...
                print(f"New best model saved at epoch {self.best_dice_epoch} with dice: {self.best_dice:.4f}")
            ...
            if self.epoch % self.args.save_epoch == 0:
                checkpoint = {
                        ...
                        'best_dice': meters['dice'].avg,
                        'num_params': self.num_params
                    }
                torch.save(checkpoint, os.path.join(self.args.save_dir, 'checkpoint-%d.pth' % self.epoch))
                print(f"New checkpoint saved at epoch {self.epoch} with dice: {meters['dice'].avg:.4f}")
...
class Trainer:
    ...
    def train_one_epoch(self):
        ...
        for inx, (img, mask, label, clinical) in tqdm(enumerate(self.train_loader), total=len(self.train_loader)):
            ...
            if not self.use_clip:
                clinical = torch.zeros_like(clinical)
            cls = self.model(img, mask.float())
            bce_loss = self.calculate_loss(cls, label)
            loss = 1 * bce_loss
            self.optimizer.zero_grad()
            loss.backward()
            self.optimizer.step()
